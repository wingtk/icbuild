...
from icbuild.errors import FatalError, CommandError, SkipToPhase, SkipToEnd
...
class BuildScript:
    def __init__(self, config, module_list=None, module_set=None):
        ...
        if not os.access(self.config.prefix, os.R_OK|os.W_OK|os.X_OK):
            raise FatalError('install prefix (%s) must be writable' % self.config.prefix)
        ...
        if not os.path.exists(self.config.checkoutroot):
            try:
                ...
                raise FatalError(
                        'checkout root (%s) can not be created' % self.config.checkoutroot)
        if not os.access(self.config.checkoutroot, os.R_OK|os.W_OK|os.X_OK):
            raise FatalError('checkout root (%s) must be writable' % self.config.checkoutroot)
        ...
        if self.config.copy_dir and not os.path.exists(self.config.copy_dir):
            try:
                ...
                raise FatalError(
                        'checkout copy dir (%s) can not be created' % self.config.copy_dir)
            if not os.access(self.config.copy_dir, os.R_OK|os.W_OK|os.X_OK):
                raise FatalError('checkout copy dir (%s) must be writable' % self.config.copy_dir)
            ...
            for dep in module.dependencies:
                if dep in failures:
                    if self.config.module_nopoison.get(dep,
                        ...
                        self.message('module %(mod)s will be built even though %(dep)s failed'
                                     % { 'mod':module.name, 'dep':dep })
                    else:
                        self.message('module %(mod)s not built due to non buildable %(dep)s'
                                     % { 'mod':module.name, 'dep':dep })
                        failed = True
            if failed:
