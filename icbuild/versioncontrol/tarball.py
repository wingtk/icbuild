...
from icbuild.errors import FatalError, CommandError, BuildStateError
from icbuild.versioncontrol import Repository, Branch, register_repo_type
from icbuild.utils.cmds import has_command, get_output
from icbuild.modtypes import get_branch
from icbuild.utils.unpack import unpack_archive
...
class TarballRepository(Repository):
    ...
    def branch(self, name, version, module=None, checkoutdir=None,
        ...
        if name in self.config.branches:
            ...
            if not module:
                raise FatalError('branch for %(name)s has wrong override, check your %(filename)s' % \
                                 {'name'     : name,
...
class TarballBranch(Branch):
    ...
    def _check_tarball(self):
        ...
        if self.source_hash is not None:
            try:
                ...
                logging.warning('invalid hash attribute on module %s' % self.module)
                return
            if hasattr(hashlib, algo):
                ...
                logging.warning('skipped hash check (missing support for %s)' % algo)
    ...
    def _download_and_unpack(self, buildscript):
        ...
        if not os.path.exists(self.config.tarballdir):
            try:
                ...
                raise FatalError(
                        'tarball dir (%s) can not be created' % self.config.tarballdir)
        if not os.access(self.config.tarballdir, os.R_OK|os.W_OK|os.X_OK):
            raise FatalError('tarball dir (%s) must be writable' % self.config.tarballdir)
        try:
            self._check_tarball()
        ...
        if not os.path.exists(self.srcdir):
            raise BuildStateError('could not unpack tarball (expected %s dir)'
    ...
    def _do_patches(self, buildscript):
        ...
        for (patch, patchstrip) in self.patches:
            ...
            if not patchfile:
                # nothing else, use icbuild provided patches
                possible_locations = []
                ...
                for dirname in possible_locations:
                    ...
                    raise CommandError('Failed to find patch: %s' % patch)
