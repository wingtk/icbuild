...
import icbuild.utils.fileutils as fileutils
...
def parse_xml_node(node, config, uri, repositories, default_repo):
    if not node.nodeName in _module_types:
        try:
            __import__('icbuild.modtypes.%s' % node.nodeName)
        except ImportError:
            pass
    if not node.nodeName in _module_types:
    ...
    def add_to_system_dependencies(lst, childnode):
        for dep in childnode.childNodes:
            if dep.nodeType == dep.ELEMENT_NODE and dep.nodeName == 'dep':
                ...
                if not typ:
                    raise FatalError('%(node)s node for %(module)s module is'
        ...
        for name in os.listdir(path):
            ...
            if os.path.isdir(subpath):
                self._clean_la_files_in_dir(buildscript, subpath)
                    ...
                    try:
                        logging.info('Deleting .la file: %r' % (subpath, ))
                        os.unlink(subpath)
                    except OSError:
                        pass
    ...
    def _clean_texinfo_dir_files(self, buildscript, installroot):
        ...
        if os.path.isdir(prefixdir):
            ...
            if os.path.isfile(dirfile):
                try:
                    logging.info('Deleting dir file: %r' % (dirfile, ))
                    os.unlink(dirfile)
                except OSError:
                    pass
...
class DownloadableModule:
    ...
    do_force_checkout.label = 'wipe directory and start over'
...
register_lazy_module_type('autotools', 'icbuild.modtypes.autotools')
register_lazy_module_type('cvsmodule', 'icbuild.modtypes.autotools')
register_lazy_module_type('svnmodule', 'icbuild.modtypes.autotools')
